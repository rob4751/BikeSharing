...
<body class="bg-gray-800 text-gray-200 font-sans">
    ...
    <div id="main-content" class="lg:ml-64 min-h-screen transition-all duration-300">
        ...
        <div id="profile-content" class="p-4 md:p-6">
            <h1 class="text-2xl font-semibold mb-6">Impostazioni Profilo</h1>

            <!-- User Profile Card - Updated -->
            <div id="profile-card" class="bg-gray-900 rounded-xl p-4 md:p-6 mb-6 shadow-lg">
                <div class="flex flex-col sm:flex-row items-center sm:items-center justify-between">
                    <div class="flex flex-col sm:flex-row items-center mb-4 sm:mb-0">
                        <?php
                            $initials = strtoupper(substr($utente['nome'], 0, 1) . substr($utente['cognome'], 0, 1));
                        ?>
                        <div class="w-16 h-16 rounded-full bg-accent-blue flex items-center justify-center text-white font-medium text-2xl">
                            <?php echo $initials; ?>
                        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
                <!-- Email Information -->
                <div id="email-section" class="bg-gray-900 rounded-xl p-6 shadow-lg">
                    <div class="flex justify-between items-center mb-4">
                        <h2 class="text-lg font-semibold text-white">Indirizzo Email</h2>
                    </div>

                    <div class="space-y-4">
                        <div class="flex justify-between items-center p-3 bg-gray-800 rounded-lg">
                            <div>
                                <p class="text-white"><?php echo htmlspecialchars($utente['email']); ?></p>
                                <div class="flex items-center mt-1">
                                    <span
                                        class="text-xs px-2 py-0.5 bg-accent-blue bg-opacity-20 text-accent-blue rounded-full mr-2">Principale
                                    </span>
                                </div>
                            </div>
                        </div>
                ...
                <div id="plan-section" class="bg-gray-900 rounded-xl p-6 shadow-lg">
                    <div class="flex justify-between items-center mb-4">
                        <h2 class="text-lg font-semibold text-white">Piano corrente</h2>
                    </div>

                    <div class="bg-gray-800 rounded-lg p-4 flex justify-between items-center">
                        <div>
                            <p class="text-white text-base font-medium">
                                <?php echo htmlspecialchars($utente['piano_attuale'] ?? 'Piano gratis'); ?>
                            </p>
                            <p class="text-sm text-gray-400 mt-1">
                                Goditi i servizi del <?php echo htmlspecialchars($utente['piano_attuale'] ?? 'Piano gratis'); ?>.
                            </p>
                        </div>
                        <a href="index.php#pricing" class="bg-accent-blue hover:bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-semibold">
                            Migliora
                        </a>
                    </div>
                </div>
